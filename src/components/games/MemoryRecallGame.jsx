...
export default function MemoryRecallGame({ profileId = 'default_patient', patientProfile = null, initialTier = 1, onComplete = null, onExit = null }) {
  ...
  const getLabel = (option) => isEnglish ? option.labelEn : isHindi ? option.labelHi || option.labelEn : option.labelAs;
  ...
  const handleSelect = (option) => {
    ...
    if (option.id === task.correctAnswer) {
      ...
      setShowSuccess(true);
      recordBiomarkerEvent({ profileId, taskType: task.id, latencyMs: 0, errorCount: eliminated.length, ddaAdjustment: 'maintained' });
    ...
    <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-soft max-w-2xl mx-auto space-y-5">
      ...
      <div className="text-center space-y-2">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <button type="button" aria-label="Listen to question" onClick={handleSpeak} className="w-10 h-10 rounded-full border border-slate-200">🔊</button>
        </div>
      </div>
      {showSuccess && <div data-testid="success-banner" className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-center font-semibold">🌟 {isEnglish ? 'Correct answer!' : 'নিখুঁত মিল! আপোনাৰ বাছনি শুদ্ধ হৈছে।'}</div>}
      {!showSuccess && eliminated.length > 0 && <div data-testid="gentle-hint-banner" className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-700">💡 {isEnglish ? 'Let us try another instrument.' : task.gentlePrompt}</div>}
      <div className="grid gap-3">
        {task.options.map(option => (
          <button key={option.id} type="button" disabled={selected === option.id || eliminated.includes(option.id) || showSuccess} onClick={() => handleSelect(option)} className="min-h-[60px] rounded-2xl border-2 border-slate-200 bg-white text-lg font-semibold hover:bg-teal-50 disabled:opacity-40">
            <span className="mr-2">{option.icon}</span>{getLabel(option)}
          </button>
        ))}
      </div>
    </div>
